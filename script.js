...
const ratingGraph = (arr) => {
  const line_con = document.querySelector(
    "g.highcharts-series.highcharts-series-0.highcharts-line-series > path.highcharts-graph"
  );
  const lines = document.querySelectorAll(
    "g.highcharts-series.highcharts-series-0.highcharts-line-series > path.highcharts-graph .highcharts-point"
  );
...
const heatmap = () => {
  ...
  sel.forEach((el) =>
    el.setAttribute(
      "fill",
      fill_colors[Math.floor(Math.random() * fill_colors.length)]
    )
  );
};
//-----------------------------------------------------
...
Z`
  ...
  bars.forEach((bar, idx) => {
    ...
    bar.setAttribute(
      "d",
      makeD(idx, min_h + (arr[idx * 5] / 255) * (max_h - min_h))
    );
  });
};
// -----------------------------------------------------
...
`;
  const meter_container = document.querySelector(
...
const meter = (arr) => {
  ...
  meter_el.setAttribute(
    "class",
    `fill-transparent qa_6R stroke-sd-${
      arr[0] < 100 ? "easy" : arr[0] < 200 ? "medium" : "hard"
    }`
  );
  meter_el.setAttribute(
    "style",
    `stroke-width: 3; stroke-linecap: round; stroke-dasharray: 315; stroke-dashoffset: ${
      315 - (arr[0] / 255) * 260
    };`
  );
};
...
<g class="origin-center translate-x-0" style="rotate:90deg">
    ...
    <circle cx="50" cy="50" r="42" class="fill-transparent qa_6R stroke-sd-${
      arr[0] < 100 ? "easy" : arr[0] < 200 ? "medium" : "hard"
    }" style="stroke-width: 3; stroke-linecap: round; stroke-dasharray: 315; stroke-dashoffset: ${
    315 - (arr[0] / 255) * 260
  };">
    </circle>
</g>
...
const getGifBadgeUrl = (month, year) => {
  month = +month;
  year = +year;
  return `https://assets.leetcode.com/static_assets/public/images/badges/${year}/gif/${year}-${
    month < 10 ? "0" : ""
  }${month}.gif`;
};
...
const animateBadges = () => {
  ...
  const profile_badge = document.querySelector(
    "div[class='ml-1'] img[class='h-[12px]']"
  );
...
const animateOthers = () => {
  const streak_days = document.querySelector(
    "#headlessui-popover-button-\\:r2\\:"
  );
  ...
  profile_img_left?.setAttribute(
    "src",
    `${server_base_url}/alan-walker-glitch.gif`
  );
};

const audio_container = document.querySelector(
  "#__next > div.flex.min-h-screen.min-w-\\[360px\\].flex-col.text-label-1.dark\\:text-dark-label-1.bg-layer-bg.dark\\:bg-dark-layer-bg > div.mx-auto.w-full.grow.p-4.md\\:max-w-\\[888px\\].md\\:p-6.lg\\:max-w-screen-xl > div > div.lc-lg\\:max-w-\\[calc\\(100\\%_-_316px\\)\\].w-full > div.lc-xl\\:flex-row.lc-xl\\:space-y-0.flex.w-full.flex-col.space-x-0.space-y-4.lc-xl\\:space-x-4 > div.bg-layer-1.dark\\:bg-dark-layer-1.shadow-down-01.dark\\:shadow-dark-down-01.rounded-lg.h-\\[180px\\].w-full.flex-1 > div > div"
);
